...
class CrossbarArbiterSimulationTest(numberOfMasters: Int, numberOfSlaves: Int) extends AnyFlatSpec
	with LightweightSimulationFixture[CrossbarArbiterFixture]
	with Inspectors {

	protected override def dutFactory() = new CrossbarArbiterFixture(numberOfMasters, numberOfSlaves)
	...
	"CrossbarArbiter slaves after reset" must "have the index of the granted master zeroed" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.encoder.isValid #= true
			slave.masters.foreach(_.request #= true)
			fixture.reset()
			slave.grantedMasterIndex.toInt must be(0)
		}
	}

	they must "have no master granted access to the bus when no master is requesting control, unless there is only one master" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.encoder.isValid #= true
			slave.masters.foreach(_.request #= false)
			fixture.reset()
			forAll(slave.masters) { master => master.isGranted.toBoolean must be(numberOfMasters == 1) }
		}
	}

	they must "have no master stalled waiting for access to the bus when no master is requesting control" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.encoder.isValid #= true
			slave.masters.foreach(_.request #= false)
			fixture.reset()
	...
	they must "have all masters except the first stalled waiting for access to the bus when all masters are requesting control" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.encoder.isValid #= true
			slave.masters.foreach(_.request #= true)
			fixture.reset()
	...
	they must "have no master with an error flag" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.encoder.isValid #= true
			slave.masters.foreach(_.request #= true)
			fixture.reset()
	...
	if (numberOfMasters > 1) {
		"CrossbarArbiter with multiple masters" must "latch the index of the single master requesting control when the encoder output is valid" in simulator { fixture =>
			forAll(fixture.io.slaves) { slave =>
				...
				slave.encoder.isValid #= true
				slave.masters.zipWithIndex.foreach { case (master, masterIndex) => master.request #= masterIndex == indexOfMasterRequestingControl }
				fixture.reset()
				fixture.clockActive()
				sleep(1)
		...
		it must "not latch the index of the next master requesting control until relinquished" in simulator { fixture =>
			forAll(fixture.io.slaves) { slave =>
				...
				slave.encoder.isValid #= true
				slave.masters.zipWithIndex.foreach { case (master, masterIndex) => master.request #= masterIndex == indexOfFirstMasterRequestingControl }

				fixture.reset()
				fixture.clock()
				slave.masters(indexOfSecondMasterRequestingControl).request #= true
		...
		it must "latch the index of the next master requesting control when the encoder output is valid and control has been relinquished" in simulator { fixture =>
			forAll(fixture.io.slaves) { slave =>
				...
				fixture.reset()
				fixture.clock()
				slave.masters(indexOfFirstMasterRequestingControl).request #= false
				slave.masters(indexOfSecondMasterRequestingControl).request #= true
				slave.encoder.output #= indexOfSecondMasterRequestingControl
				fixture.clockActive()
				sleep(1)
		...
		"CrossbarArbiter with single master" must "hard-code the granted master when the encoder output is valid and the master is not requesting control" in simulator { fixture =>
			forAll(fixture.io.slaves) { slave =>
				slave.encoder.output #= 0
				slave.encoder.isValid #= true
				slave.masters(0).request #= false
				fixture.reset()
				fixture.clock()
				slave.grantedMasterIndex.toInt must be(0)
			}
	...
	"CrossbarArbiter encoder inputs" must "all be false when no masters are requesting to be granted bus access, unless there is only one master" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.masters.foreach(_.request #= false)
			sleep(1)
	...
	they must "be the asynchronous value of the master request lines, unless there is only one master" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			...
			slave.masters.zip(requests).foreach { case (master, request) => master.request #= request }
			sleep(1)
			slave.encoder.inputs.map(_.toBoolean) must be(if (numberOfMasters == 1) Seq(true) else requests)
		}
	}

	"CrossbarArbiter masters" must "not be granted control when they are not requesting it, unless there is only one master" in simulator { fixture =>
		forAll(fixture.io.slaves) { slave =>
			slave.encoder.isValid #= true
			slave.masters.foreach(_.request #= false)
			fixture.reset()
	...
	private def notHaveAnErrorWhenTheyAreNotRequestingControlAndTheEncoderOutputIs(isValid: Boolean)(implicit fixture: CrossbarArbiterFixture) = {
		forAll(fixture.io.slaves) { slave =>
		...
		if (numberOfMasters > 1) {
			forAll(fixture.io.slaves) { slave =>
				...
				forAll(slave.masters.zipWithIndex) { case (master, masterIndex) =>
					...
					slave.masters(anotherMaster).request #= true
		...
		if (numberOfMasters > 1) {
			forAll(fixture.io.slaves) { slave =>
				...
				forAll(slave.masters.zipWithIndex) { case (master, masterIndex) =>
					...
					slave.encoder.output #= anotherMaster
					fixture.clock()
					slave.masters(anotherMaster).request #= false
					slave.encoder.output #= masterIndex
					master.request #= true
					fixture.clock()
					master.isGranted.toBoolean must be(true)
