...
class Ice40Spram16k16WishboneBusAdapterTest extends AnyFlatSpec with NonSimulationFixture {
	"Ice40Spram16k16WishboneBusAdapter" must "not use the 'io' prefix for signals" in spinalContext {
		val adapter = new Ice40Spram16k16WishboneBusAdapter()
		adapter.io.name must be("")
	}

	"Ice40Spram16k16WishboneBusAdapter Wishbone bus" must "have a 14-bit address width" in spinalContext {
		val adapter = new Ice40Spram16k16WishboneBusAdapter()
		adapter.io.wishbone.ADR.getWidth must be(14)
	}
	...
	it must "have a 4-bit write-mask width" in spinalContext {
		val adapter = new Ice40Spram16k16WishboneBusAdapter()
		adapter.io.wishbone.SEL.getWidth must be(4)
	}
