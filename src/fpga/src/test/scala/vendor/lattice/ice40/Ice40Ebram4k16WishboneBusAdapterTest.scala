...
class Ice40Ebram4k16WishboneBusAdapterTest extends AnyFlatSpec with LightweightSimulationFixture[Ice40Ebram4k16WishboneBusAdapterFixture] {
	protected override def dutFactory() = new Ice40Ebram4k16WishboneBusAdapterFixture()

	"Ice40Ebram4k16WishboneBusAdapter" must "not use the 'io' prefix for signals" in simulator { fixture =>
		val adapter = new Ice40Ebram4k16WishboneBusAdapter()
		adapter.io.name must be("")
	}

	"Ice40Ebram4k16WishboneBusAdapter Wishbone bus" must "have an 8-bit address width" in simulator { fixture =>
		fixture.io.wishbone.ADR.getWidth must be(8)
	}
	...
	it must "have a 16-bit write-mask width" in simulator { fixture =>
		fixture.io.wishbone.SEL.getWidth must be(16)
	}
	...
	it must "not assert EBRAM CER when reading and Wishbone CYC is low" in simulator { fixture =>
